...
class StatisticsServiceImplTest {
	...
	void 월간통계획득_성공() {
		...
		for (CategoryBig categoryBig : categoryBigs) {
			...
			for (int i = 1; i <= 6; i++) {
				CategoryDetail tcd = testEntityFactory.getCategoryDetail(
					(Integer.parseInt(categoryBig.getName()) + i) + "소분류", i + "번이미지", categoryBig);
				for (int j = 0; j < 10; j++) {
					Food tfe = testEntityFactory.getDeletedFood("f1", 5000, refrigerator, StorageType.builder().build(),
						tcd, LocalDateTime.now(), LocalDateTime.now(), FoodDeleteType.EATEN, LocalDateTime.now());
					Food tft = testEntityFactory.getDeletedFood("f2", 5000, refrigerator, StorageType.builder().build(),
						tcd, LocalDateTime.now(), LocalDateTime.now(), FoodDeleteType.THROWN, LocalDateTime.now());
					foodList.add(tfe);
					foodList.add(tft);
				}
